...
def get_qasrl_annots_post_parser(parer_output_file):
    '''
    ...
    for i, qa in enumerate(qasrl):
        ...
        for verb_obj in qa["verbs"]:
            ...
            for qa_pair in verb_obj["qa_pairs"]:
                row = (qasrl_id, verb, qa_pair["question"], qa_pair["spans"][0]["text"],
                       str(qa_pair["spans"][0]["start"]) + ":" + str(qa_pair["spans"][0]["end"]), verb_ind)
                qas.append(row)

    qasrl_qas = pd.DataFrame(qas)
    qasrl_qas.reset_index(inplace=True)
    qasrl_qas.columns = ['qa_uuid', 'qasrl_id', 'verb', 'question', 'answer',
...
def get_qas_per_qasrlid(df):
    ...
    qaid2qas_d = {}
    for i, g in grouped:
        qa_zip = list(zip(g["qa_uuid"], g["verb"],g["question"],
                          g["answer"]))
        ...
        for item in qa_zip:
            ...
            list_qas_d.append({"qa_uuid": item[0], "verb":item[1], "question": item[2],
                     "answer": item[3]})
        qaid2qas[i] = list_qas
